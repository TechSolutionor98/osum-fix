...
export function parsePageContent(pageFilePath, slug = null) {
  ...
  function parseFile(filePath, componentName) {
    ...
    if (Object.keys(sectionFields).length > 0) {
      ...
      // Check for section comments to split into multiple dropdowns
      const sectionRegex = /\{\/\*\s*(?:Section\s*\d*:\s*([^({]+)|([^({]+))\s*(?:\([^)]+\))?\s*\*\/\}/gi;
      const customSections = [];
      let secMatch;
      while ((secMatch = sectionRegex.exec(rawContent)) !== null) {
        let rawName = (secMatch[1] || secMatch[2]).trim().split('(')[0].trim();
        
        // Map to exactly what the user requested
        let mappedName = rawName;
        if (slug === 'ac-work') {
          if (rawName.includes('Overview') || rawName.includes('Hero')) mappedName = 'AC Repair & Maintenance Services';
          ...
          if (rawName.includes('Overview') || rawName.includes('Hero')) mappedName = 'Premium AC and Maintenance Services';
          else if (rawName.includes('Why Choose Us') || rawName.includes('Why OsumFix')) mappedName = 'Why OsumFix';
          ...
          else if (rawName.includes('WhatsApp')) mappedName = 'Contact via WhatsApp';
        }
      ...
      if (customSections.length > 0 && slug) {
        ...
        const defaultGroupName = componentName;
        grouped[defaultGroupName] = {};

        sortedFieldsArr.forEach(({ key, field }) => {
          let targetSec = defaultGroupName;
        ...
        Object.keys(grouped).forEach(secName => {
          if (Object.keys(grouped[secName]).length > 0) {
            sections.push({
