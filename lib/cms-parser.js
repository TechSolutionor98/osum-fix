...
function isCodeOrStyleString(val) {
  ...
  // 2. Skip file paths, routes, and imports (e.g. "@/...", "./...", "../...", "/images/...", "components/...")
  ...
  // 7. Skip Tailwind utility classes (if the string is a sequence of styling classes)
  if (trimmed.includes(' ')) {
    const tokens = trimmed.split(/\s+/);
    // Standard utility class prefix patterns (bg-*, text-*, spacing, flex, grids, space, animate)
    const styleClassRegex = /^(bg|text|px|py|p|pt|pb|pl|pr|m|mt|mb|ml|mr|mx|my|w|h|border|rounded|shadow|gap|col|row|translate|scale|opacity|duration|ease|font|tracking|leading|items|justify|overflow|pointer|select|flex|grid|min-h|max-h|min-w|max-w|aspect|z|from|via|to|delay|origin|cursor|fill|stroke|space|animate)-[a-z0-9]/i;
    const styleWordRegex = /^(flex|grid|block|inline|hidden|absolute|relative|static|fixed|sticky|group|pointer|select|truncate|transition|duration|ease|delay|cursor|overflow|aspect)$/i;
    
    const styleTokens = tokens.filter(t => 
      t.includes(':') || 
      styleClassRegex.test(t) || 
      styleWordRegex.test(t)
    );
    
    // If more than 40% of tokens match layout/styling classes, it's a styles chain
    if (styleTokens.length / tokens.length > 0.4) {
      return true;
    }
  }
...
export function parsePageContent(pageFilePath) {
  ...
  function parseFile(filePath, componentName) {
    ...
    // 1. Scan JSX tags for plain static texts (including spans, divs, list items, etc.)
    ...
    // 2. Scan JS object string properties (e.g. inside lists, reason arrays, product specs)
    let jsIdx = 1;
    ...
    // 2.5 Scan all general string literals in the file (to catch features, specifications, list items, etc.)
    let strIdx = 1;
    const strRegex = /(['"`])((?:[^\\]|\\.)*?)\1/g;
    let strMatch;
