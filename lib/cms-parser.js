...
function getServiceBlock(fileContent, slug) {
  // Find where slug === "slug" is declared
  const searchStr = `slug === "${slug}"`;
  let startIdx = fileContent.indexOf(searchStr);
  if (startIdx === -1) {
    startIdx = fileContent.indexOf(`slug === '${slug}'`);
  }
  if (startIdx === -1) return fileContent;

  // Find the start of "const is" before startIdx
  const beforeSnippet = fileContent.substring(0, startIdx);
  const constIdx = beforeSnippet.lastIndexOf('const is');
  if (constIdx !== -1 && startIdx - constIdx < 100) {
    startIdx = constIdx;
  }

  // Find the next service block declaration: const isSomeVar = slug === "some-slug"
  let endIdx = fileContent.length;
  const substringForSearch = fileContent.substring(startIdx + 50); // skip the current declaration line
  const nextMatch = substringForSearch.match(/const\s+is[A-Za-z0-9_]+\s*=\s*slug\s*===\s*['"][a-z0-9\-]+['"]/);
  if (nextMatch) {
    endIdx = startIdx + 50 + nextMatch.index;
  } else {
    // If no next service block, search for the fallback block
    const fallbackIdx = fileContent.indexOf('// Fallback dynamic rendering', startIdx);
    if (fallbackIdx !== -1) {
      endIdx = fallbackIdx;
    }
  }
 ...
 * @param {string|null} slug - The service slug to isolate fields for
...
export function parsePageContent(pageFilePath, slug = null) {
  ...
  function parseFile(filePath, componentName) {
    ...
    // Isolate service block before any processing to ensure rawContent is scoped correctly
    if (slug && filePath === pageFilePath) {
      rawContent = getServiceBlock(rawContent, slug);
    }
    ...
    if (Object.keys(sectionFields).length > 0) {
      ...
      // Check for section comments to split into multiple dropdowns matching visual client-side layout
      const sectionRegex = /\{\/\*\s*Section\s*\d*:\s*([^({]+)\s*(?:\([^)]+\))?\s*\*\/\}/gi;
      const customSections = [];
      let secMatch;
      while ((secMatch = sectionRegex.exec(rawContent)) !== null) {
        let rawName = secMatch[1].trim().split('(')[0].trim();
        
        // Map to clean readable names matching client side top to bottom
        let mappedName = rawName;
        const formattedSlug = slug ? slug.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') : '';
        if (slug === 'ac-work') {
          if (rawName.includes('Overview') || rawName.includes('Hero')) mappedName = 'AC Repair & Maintenance Services (Hero)';
          else if (rawName.includes('Why Choose Us') || rawName.includes('Why OsumFix')) mappedName = 'Why OsumFix';
          else if (rawName.includes('Maintenance Process') || rawName.includes('Method')) mappedName = 'Our Methodology';
          else if (rawName.includes('Residential')) mappedName = 'Residential & Commercial AC Solutions';
          else if (rawName.includes('Explore')) mappedName = 'Explore Our Other Services';
          else if (rawName.includes('Services') || rawName.includes('Repairs') || rawName.includes('Expertise')) mappedName = 'Our Services';
          else if (rawName.includes('Process') || rawName.includes('Workflow') || rawName.includes('Bookings')) mappedName = 'Our Workflow';
          else if (rawName.includes('FAQ')) mappedName = 'FAQ';
        } else {
          if (rawName.includes('Overview') || rawName.includes('Hero')) mappedName = formattedSlug ? `${formattedSlug} Overview (Hero)` : 'Overview (Hero)';
          else if (rawName.includes('Why Choose Us') || rawName.includes('Why OsumFix')) mappedName = 'Why OsumFix';
          else if (rawName.includes('Maintenance Process') || rawName.includes('Method')) mappedName = 'Our Method';
          else if (rawName.includes('Explore')) mappedName = 'Explore Other Services';
          else if (rawName.includes('Services') || rawName.includes('Repairs') || rawName.includes('Expertise')) mappedName = formattedSlug ? `Our ${formattedSlug} Services` : 'Our Services';
          else if (rawName.includes('Process') || rawName.includes('Workflow') || rawName.includes('Bookings')) mappedName = 'Our Workflow';
          else if (rawName.includes('Emergency') || rawName.includes('Specialties') || rawName.includes('Sectors')) mappedName = 'Emergency & Sectors';
          else if (rawName.includes('FAQ')) mappedName = 'FAQ';
          else if (rawName.includes('WhatsApp') || rawName.includes('Areas We Serve')) mappedName = 'Areas We Serve & Contact';
        }

        customSections.push({ index: secMatch.index, name: mappedName });
      }

      // We only apply this custom splitting for dynamic slug pages
      if (customSections.length > 0 && slug) {
        const grouped = {};
        customSections.forEach(cs => { grouped[cs.name] = {}; });
        const sidebarNavName = 'Sidebar Navigation';
        grouped[sidebarNavName] = {};

        sortedFieldsArr.forEach(({ key, field }) => {
          let targetSec = customSections[0].name;
          if (field.index < customSections[0].index) {
            targetSec = sidebarNavName;
          } else {
            for (let i = customSections.length - 1; i >= 0; i--) {
              if (customSections[i].index <= field.index) {
                targetSec = customSections[i].name;
                break;
              }
            }
          }
          const { index, ...rest } = field;
          grouped[targetSec][key] = rest;
        });

        let orderOffset = 0;
        customSections.forEach(cs => {
          const secName = cs.name;
          if (grouped[secName] && Object.keys(grouped[secName]).length > 0) {
            if (!sections.some(s => s.sectionName === secName)) {
              sections.push({
                sectionId: secName.toLowerCase().replace(/[^a-z0-9]/g, '_'),
                sectionName: secName,
                filePath: relativePath,
                order: sections.length + 1 + orderOffset++,
                fields: grouped[secName],
              });
            }
          }
        });

        if (grouped[sidebarNavName] && Object.keys(grouped[sidebarNavName]).length > 0) {
          sections.push({
            sectionId: 'sidebar_navigation',
            sectionName: sidebarNavName,
            filePath: relativePath,
            order: sections.length + 1 + orderOffset++,
            fields: grouped[sidebarNavName],
          });
        }
      } else {
        const sortedFields = {};
  ...
  try {
    ...
    // Handle specific Home Page mapping
    const isHomePage = pageFilePath.replace(/\\/g, '/').endsWith('/app/page.tsx') || pageFilePath.replace(/\\/g, '/').endsWith('/app/page.js');
    if (isHomePage) {
      const homeMapping = {
        'Hero': { name: 'Hero Section', order: 1 },
        'Services': { name: 'Popular Services', order: 2 },
        'WhyChooseUs': { name: 'Resolution in 3 Simple Steps', order: 3 },
        'Projects': { name: 'Project Excellence', order: 4 },
        'VideoSection': { name: 'Experience the OsumFix Difference', order: 5 },
        'FAQSection': { name: 'Got Questions? Frequently Asked Questions', order: 6 },
      };

      const filteredSections = [];
      for (const s of sections) {
        const mapping = homeMapping[s.sectionName];
        if (mapping) {
          s.sectionName = mapping.name;
          s.order = mapping.order;
    ...
    // Handle specific About Page mapping
    const isAboutPage = pageFilePath.replace(/\\/g, '/').endsWith('/app/about/page.tsx') || pageFilePath.replace(/\\/g, '/').endsWith('/app/about/page.js');
    if (isAboutPage) {
      const aboutMapping = {
        'WhoWeAre': { name: 'Who We Are', order: 1 },
        'Stats': { name: 'Statistics Section', order: 2 },
        'Philosophy': { name: 'Our Purpose & Values', order: 3 },
        'Commitment': { name: 'Our Standard', order: 4 },
        'Advantages': { name: 'Our Advantages', order: 5 },
        'Team': { name: 'Our People', order: 6 },
      };

      const filteredSections = [];
      for (const s of sections) {
        const mapping = aboutMapping[s.sectionName];
        if (mapping) {
          s.sectionName = mapping.name;
          s.order = mapping.order;
          filteredSections.push(s);
    ...
    // Handle specific Services Page mapping
    const isServicesPage = pageFilePath.replace(/\\/g, '/').endsWith('/app/services/page.tsx') || pageFilePath.replace(/\\/g, '/').endsWith('/app/services/page.js');
    if (isServicesPage) {
      const allServicesSec = { sectionId: 'all_services', sectionName: 'All Services', filePath: pageFilePath, order: 1, fields: {} };
      const annualPackageSec = { sectionId: 'annual_package', sectionName: 'Annual Package', filePath: pageFilePath, order: 2, fields: {} };
      const serviceCardsSec = { sectionId: 'service_cards', sectionName: 'Service Cards', filePath: pageFilePath, order: 3, fields: {} };

      let allIdx = 1, annIdx = 1, cardIdx = 1;
      const seenAll = new Set(), seenAnn = new Set(), seenCard = new Set();

      for (const section of sections) {
        if (section.sectionId === 'cta') continue;

        for (const [key, field] of Object.entries(section.fields)) {
          const val = field.originalValue || '';
          if (!val) continue;

          if (['Annual Package', 'Total Home Care', 'Unlimited call-outs and priority response for one low monthly fee. Secure your peace of mind.', '24/7 Priority Support', 'Free Parts Replacement', 'Bi-annual AC Servicing', 'No Hidden Call-out Fees', 'Learn More'].includes(val)) {
            if (!seenAnn.has(val)) {
              seenAnn.add(val);
              annualPackageSec.fields[`f_${annIdx++}`] = field;
            }
          } else if (['Our Services', 'Comprehensive technical solutions tailored for premium residences and commercial properties across Dubai. Reliable, certified, and available 24/7.', 'All Services', 'AC', 'Electrical', 'Plumbing', 'Painting', 'Masonry', 'Carpentry', 'Steel Fixing', 'Interior', 'Ceiling', 'Handyman'].includes(val)) {
            if (!seenAll.has(val)) {
              seenAll.add(val);
              allServicesSec.fields[`f_${allIdx++}`] = field;
            }
          } else {
            if (!seenCard.has(val)) {
              seenCard.add(val);
              serviceCardsSec.fields[`f_${cardIdx++}`] = field;
            }
          }
        }
      }

      return [allServicesSec, annualPackageSec, serviceCardsSec];
    }
