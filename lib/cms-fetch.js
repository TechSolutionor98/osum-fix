...
export async function getCmsSeo(path, websiteId = 'default') {
  try {
    ...
    // Find matching route by path to get its routeId
    const route = await db.collection('cms_routes').findOne({ path, websiteId });
    
    const filterConditions = [{ path }];
    if (route) {
      filterConditions.push({ routeId: route._id.toString() });
      filterConditions.push({ routeId: route._id });
    }
...
export async function generateCmsMetadata(path, defaults = {}, websiteId = 'default') {
  ...
  if (seo.twitterCard) {
    ...
    if (seo.twitterCard.image && seo.twitterCard.image.trim()) {
