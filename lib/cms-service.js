...
export async function getDashboardCounts() {
  ...
  const [appCount, contactCount, reviewCount, mediaCount, routesList] = await Promise.all([
...
export async function getPageContent(routeId, websiteId = 'default') {
  ...
  const shouldExcludeSection = (sectionId, sectionName) => {
    ...
    if (isHomePage) {
      const allowedHomeSections = ['hero', 'services', 'whychooseus', 'projects', 'videosection', 'faqsection'];
      if (!allowedHomeSections.includes(sid)) {
        return true;
      }
    }

    const isAboutPage = route && route.path === '/about';
    if (isAboutPage) {
      const allowedAboutSections = ['whoweare', 'stats', 'philosophy', 'commitment', 'advantages', 'team'];
      if (!allowedAboutSections.includes(sid)) {
        return true;
      }
    }

    const isServicesPage = route && route.path === '/services';
    if (isServicesPage) {
      const allowedServicesSections = ['all_services', 'annual_package', 'service_cards'];
      if (!allowedServicesSections.includes(sid)) {
        return true;
      }
    }

    const isAcWorkPage = route && route.path === '/services/ac-work';
    if (isAcWorkPage) {
      const allowedAcSections = [
        'ac_repair___maintenance_services',
        'why_osumfix',
        'our_methodology',
        'our_services',
        'residential___commercial_ac_solutions',
        'our_workflow',
        'faq',
        'explore_our_other_services'
      ];
      if (!allowedAcSections.includes(sid)) {
        return true;
      }
    }
...
export async function getBlogBySlug(slug) {
  const db = await getDb();
  const blog = await db.collection('cms_blogs').findOne({ slug });
  if (!blog) return null;
