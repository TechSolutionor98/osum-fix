...
import { Home, Wrench, Info, CalendarCheck } from "lucide-react";
              ...
              >
                <div className={`transition-all duration-300 flex items-center justify-center ${
                  tab.isActive 
                    ...
                    : "w-8 h-8 bg-transparent text-slate-500"
                }`}>
                  <Icon size={22} className={tab.isActive ? "scale-110 transition-transform" : ""} />
                </div>
                <span className={`text-[10px] font-bold whitespace-nowrap transition-all duration-300 overflow-hidden ${
                  tab.isActive ? "h-0 opacity-0 scale-0 m-0" : "h-4 opacity-100 scale-100 mt-1 text-slate-500"
                }`}>
                  {tab.name}
                </span>
              </button>
